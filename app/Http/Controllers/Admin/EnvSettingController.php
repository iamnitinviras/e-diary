...
class EnvSettingController extends Controller
{
    ...
    public function update()
    {
        ...
        if ($request->has('app_dark_logo')) {
            $data['APP_DARK_LOGO'] = 'uploads/' . uploadFile($request->app_dark_logo, 'logo');
        }

        if ($request->has('app_light_logo')) {
            $data['APP_LIGHT_LOGO'] = 'uploads/' . uploadFile($request->app_light_logo, 'logo');
        }
    ...
    public function frontendImages(Request $request)
    {
        ...
        $request->validate([
            'banner_image_one' => ['max:10000', "image", 'mimes:jpeg,png,jpg,gif,svg,webp']],[
    ...
    public function seoSave()
    {
        ...
        //Save SEO Details to ENV
        $data['SEO_KEYWORD']=$request->seo_keyword;
        $data['SEO_DESCRIPTION']=$request->seo_description;

        if ($request->has('seo_image')) {
            $data['SEO_IMAGE'] = 'uploads/' . uploadFile($request->seo_image, 'seo_image');
        }
        DotenvEditor::setKeys($data)->save();
