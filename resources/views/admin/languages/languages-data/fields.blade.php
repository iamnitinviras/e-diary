...
<div class="row ">
    ...
    <div class="col-md-3">
        ...
        @endif
        <label class="form-label word-break-all">{!! $lbl_category_name !!} <span class="text-danger">*</span></label>
    </div>
    <div class="col-md-9  form-group  ">
        @if (request()->query->get('file') == 'items' && Str::contains($key, '.description'))
            {!! html()->textarea(getDotStringToInputString($key, 'other'), $other[$key] ?? $one)
            ->class('form-control')
            ->id('name')
            ->placeholder($lbl_category_name)
            ->required()
            ->rows(2)
            ->attribute('data-pristine-required-message', __('validation.custom.required'))
            ->attributes($validation) !!}
        @else
            {!! html()->text(getDotStringToInputString($key, 'other'), $other[$key] ?? $one)
            ->class('form-control')
            ->id('name')
            ->placeholder($lbl_category_name)
            ->required()
